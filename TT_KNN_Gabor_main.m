...
% -- Apply Gabor Feature Extractor to training images --
gabor_train_images = gabor(train_images)

% -- Train the model --
model = KNNTraining(gabor_train_images, train_labels)
...
% -- Apply Gabor Feature Extractor to testing images
gabor_test_images = gabor(test_images)

% -- Test the model --
k = [1,3, 5, 7,10];
for i=1:size(k,2)
    tic
    for j=1:size(gabor_test_images,1)
        test_image = gabor_test_images(j,:);
